...
export async function awardUserXp(params: AwardUserXpParams): Promise<{ ok: boolean; newTotal?: number }> {
  ...
  const { data: stateRow } = await admin.from("user_xp_state").select("total_xp, fantasy_points_tiers_claimed, login_streak, last_daily_login").eq("user_id", userId).maybeSingle();
  const prev =
    (stateRow as { total_xp?: number; fantasy_points_tiers_claimed?: number; login_streak?: number; last_daily_login?: string | null } | null) ??
    null;
