...
async function enrichWithProfiles(
  ...
  Map<string, { display_name: string | null; is_site_admin: boolean; is_suspended: boolean; suspended_until: string | null }>
> {
  const map = new Map<
    string,
    { display_name: string | null; is_site_admin: boolean; is_suspended: boolean; suspended_until: string | null }
  >();
  if (users.length === 0) return map;
  const { data, error } = await admin
    .from("profiles")
    .select("id, display_name, is_site_admin, is_suspended, suspended_until")
...
export async function siteAdminListUsers(
  ...
  if (!q) {
    ...
    const rows = await enrichWithLeagueCounts(
      admin,
  ...
  const rows = await enrichWithLeagueCounts(admin, matches.slice(start, start + perPage));
...
export async function siteAdminGetUserDetail(
  ...
  const { data: profile, error: profileErr } = await admin
    .from("profiles")
    .select(
      "display_name, avatar_url, timezone, is_site_admin, accepted_terms_at, accepted_privacy_at, is_suspended, suspended_until, suspension_reason, moderation_note"
    )
    .eq("id", userId)
    .maybeSingle();
