...
const TAG_TEAM_TO_MEMBER_SLUGS = {
  ...
  /** Raw Tag Team — member-pair slugs keep historical belt credit accurate after partner swaps */
  "logan-paul-and-austin-theory": ["logan-paul", "austin-theory"],
  "austin-theory-and-bron-breakker": ["austin-theory", "bron-breakker"],
  /** Raw Tag Team — Boxscore team slug; current Vision lineup */
  "the-vision": ["austin-theory", "bron-breakker"],
...
const RAW_TAG_VISION_MEMBER_PAIRS = [
  ["logan-paul", "austin-theory"],
  ["austin-theory", "bron-breakker"],
];

/**
 * Raw Tag reign rows sometimes list only one member slug after sync lag. Credit both listed Vision members.
...
export function expandRawTagTheVisionIfSingleMemberListed(titleName, key, slug, rawSlug) {
  ...
  for (const pair of RAW_TAG_VISION_MEMBER_PAIRS) {
    if (set.has(pair[0]) && set.has(pair[1])) return null;
  }
  if (set.has("bron-breakker")) return ["austin-theory", "bron-breakker"];
  if (set.has("logan-paul")) return ["logan-paul", "austin-theory"];
  if (set.has("austin-theory")) return ["logan-paul", "austin-theory"];
  return null;
}
