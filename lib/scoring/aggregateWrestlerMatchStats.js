...
export function aggregateWrestlerMatchStats(events, dateFrom, dateTo) {
  ...
  for (const event of sorted) {
    ...
    for (const m of scored.matches || []) {
      ...
      const resultStr = (rawMatch.result || "").toLowerCase();
      const dq = method.includes("dq") || method.includes("disqualification");
      const winnersList = m.winners ?? extractMatchParticipants(rawMatch).winners ?? [];
      const losersList = m.losers ?? extractMatchParticipants(rawMatch).losers ?? [];
      const winnerField = String(rawMatch.winner ?? rawMatch.winnerWrestler ?? rawMatch.winnerName ?? "").toLowerCase().trim();
      const nc =
        method.includes("no contest") ||
        resultStr.includes("no contest") ||
        resultStr.includes("no winner") ||
        (winnerField === "none" && winnersList.length === 0) ||
        (winnersList.length === 0 && losersList.length === 0);

      // Use winners/losers from scored match (same data used for points) so stats stay in sync
...
export function getUnparsedMatchesByWrestler(events, dateFrom, dateTo) {
  ...
  for (const event of sorted) {
    ...
    for (const m of scored.matches || []) {
      ...
      // Use winners/losers from scored match (same data used for points) so we don't mark as unparsed when scoring succeeded
      const winnerSlugs = new Set();
      const loserSlugs = new Set();
      for (const w of winnersList) {
        const name = typeof w === "string" ? w : (w && w.name) || "";
