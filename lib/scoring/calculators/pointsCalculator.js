...
export function calculateMatchPoints(match, event, allMatches, wrestlerName) {
  ...
  if (isRS) {
    ...
    if (knownSlot) {
      ...
      // Do not add to breakdown or total on R/S; these points are awarded at Night of Champions only.
      ...
      if (kotrRound === "first") {
        ...
        // Do not add to breakdown or total on R/S; qualifier +3 is awarded at Night of Champions only.
      } else if (kotrRound === "semi") {
        points.kotrTowardNOC = 7;
        points.kotrBracket = bracket;
        points.kotrRound = "semi";
        // Do not add to breakdown or total on R/S; semi +7 is awarded at Night of Champions only.
      }
    }
  }

  // DQ: win points are halved in calculateEventSpecificPoints per event type; no global halving.
  // On Raw/SmackDown, KOTR qualifier/semi points (kotrTowardNOC) are not added here; they are applied at Night of Champions.
  const addKotrToTotal = !isRS ? (points.kotrTowardNOC || 0) : 0;
