...
import { isChampionshipRetiredForScoringDate } from "../../retiredChampionships.js";
...
function isNoContest(match) {
  ...
  const winner = String(match.winner ?? match.winnerWrestler ?? match.winnerName ?? "").toLowerCase().trim();
  return (
    method.includes("no contest") ||
    result.includes("no contest") ||
    result.includes("no winner") ||
    winner === "none"
  );
}

function getVictoryBonusTierPoints(eventType) {
  if (eventType === EVENT_TYPES.NXT) return 0.5;
  if (eventType === EVENT_TYPES.NXT_STAND_AND_DELIVER) return 1.5;
  if (
    [
  ...
  if (
    ...
    return 1;
  const pleCategory = getPLECategory(eventType);
  if (pleCategory === "major") return 3;
  if (pleCategory === "minor" || pleCategory === "medium") return 2;
  return 0;
}

function getMatchTextBlob(match) {
  const matchType = getMatchType(match);
  const title = getMatchTitle(match);
  const stip = String(match?.stipulation || "");
  const result = String(match?.result || "");
  return `${matchType} ${title} ${stip} ${result}`.toLowerCase();
}

function isTwoOutOfThreeFallsLike(match) {
  const t = getMatchTextBlob(match);
  return (
    t.includes("2 out of 3") ||
    t.includes("two out of three") ||
    t.includes("2-of-3") ||
    t.includes("2/3 falls") ||
    t.includes("two falls")
  );
}

function isGauntletLike(match) {
  return getMatchTextBlob(match).includes("gauntlet");
}

function getGauntletProgression(match) {
  const progression = match?.gauntletProgression ?? match?.gauntlet_progression;
  return Array.isArray(progression) ? progression : [];
}

/** True when a progression winner field (slug, name, or "a & b" team) refers to this wrestler. */
function gauntletWinnerIncludesWrestler(winnerField, wrestlerName) {
  const wrestlerSlug = slugifyName(wrestlerName);
  if (!wrestlerSlug || winnerField == null) return false;
  const winnerStr = String(winnerField).trim();
  if (!winnerStr) return false;
  const winnerSlug = slugifyName(winnerStr);
  if (winnerSlug && winnerSlug === wrestlerSlug) return true;
  const parts = winnerStr.split(/\s*(?:&|,|\/|\+\s*| and )\s*/i).map((p) => p.trim()).filter(Boolean);
  if (parts.length <= 1) return false;
  return parts.some((part) => slugifyName(part) === wrestlerSlug);
}

/**
 * Count of gauntlet legs this wrestler (or their tag team) won from stored progression.
 * Returns null when no usable progression exists (caller may fall back).
 */
function countGauntletRoundWinsForWrestler(match, wrestlerName) {
  const progression = getGauntletProgression(match);
  if (progression.length === 0) return null;
  let wins = 0;
  for (const row of progression) {
    if (gauntletWinnerIncludesWrestler(row?.winner, wrestlerName)) wins += 1;
  }
  return wins;
}

function countMatchSides(match) {
  const raw = match?.participants;
  if (Array.isArray(raw)) return raw.filter(Boolean).length;
  const str = String(raw || "").trim();
  if (!str) return 0;
  return str.split(/\s+vs\.?\s+/i).map((s) => s.trim()).filter(Boolean).length;
}

function parseWinnerRoundWinsFromResult(match) {
  const resultText = String(match?.result || "");
  if (!resultText) return null;
  const scorePatterns = [
    /(\d+)\s*-\s*(\d+)/,
    /(\d+)\s*(?:falls?|rounds?)\s*to\s*(\d+)/i,
    /won\s+(\d+)\s*(?:falls?|rounds?)/i,
  ];
...
/**
 * Bonus for defeating more than one opponent: +tierPts per opponent beyond the first
 * (baseline match = one opponent). Triple threat / fatal four-way / handicap 2v1, etc.
 * Raw/SmackDown tier is typically 1 → e.g. beat 2 opponents → +1 special (plus normal +2 win, +1 card).
 */
function additionalOpponentVictoryBonus(match, matchData, tierPts) {
  const sideCount = countMatchSides(match);
  const qualifies = sideCount >= 3 || isHandicapMatch(match);
  if (!qualifies) return { points: 0, units: 0, reason: "" };

  const loserList = Array.isArray(matchData?.losers) ? matchData.losers : [];
  let defeatedOpponents = loserList.length;

  // Multi-team tag: losers are expanded to individuals (e.g. 6 wrestlers in a 4-way tag).
  // Count defeated sides/teams instead so bonus is per team beyond the first.
  if (isMultiTeamTagMatch(match) && sideCount >= 3) {
    defeatedOpponents = sideCount - 1;
  } else if (defeatedOpponents <= 0 && isHandicapMatch(match)) {
    const n = Array.isArray(matchData?.participantsForScoring)
      ? matchData.participantsForScoring.length
      : 0;
    if (n >= 2) defeatedOpponents = Math.max(0, n - 1);
  }
  // Handicap / uneven sides: Boxscore sometimes lists the losing team as one token in `losers`
  // (e.g. one string) even though two individuals lost. Infer defeated count from participants − winners.
  if (isHandicapMatch(match) && Array.isArray(matchData?.participantsForScoring)) {
    const parts = matchData.participantsForScoring;
    if (parts.length >= 2) {
      const winSet = new Set(
        (matchData.winners || [])
          .map((w) => slugifyName(String(w)))
          .filter(Boolean)
      );
      if (winSet.size > 0) {
        const inferredDefeated = Math.max(0, parts.length - winSet.size);
        defeatedOpponents = Math.max(defeatedOpponents, inferredDefeated);
      }
    }
  }
  if (defeatedOpponents <= 0 && sideCount >= 3) {
    defeatedOpponents = Math.max(0, sideCount - 1);
  }

  const additionalBeyondFirst = Math.max(0, defeatedOpponents - 1);
  if (additionalBeyondFirst <= 0) {
    return { points: 0, units: 0, reason: "" };
  }
  return {
    points: additionalBeyondFirst * tierPts,
    units: additionalBeyondFirst,
    reason: "additional opponents beyond the first",
  };
}

function calculateSpecialVictoryBonus(match, matchData, eventType, isWinner, wrestlerName) {
  // Battle Royal / Royal Rumble have dedicated scoring blocks; never apply generic
  // multi-person victory multipliers on top of those rules.
  if (isBattleRoyal(match) || isRoyalRumbleMatch(match)) {
    return { points: 0, units: 0, reason: "" };
  }
  const tierPts = getVictoryBonusTierPoints(eventType);
  if (tierPts <= 0) return { points: 0, units: 0, reason: "" };

  // Gauntlet: every entrant scores tier × their own round wins from progression.
  // Overall match win points (including main event) are separate via normal card scoring.
  if (isGauntletLike(match)) {
    const personalRounds = countGauntletRoundWinsForWrestler(match, wrestlerName);
    if (personalRounds != null) {
      if (personalRounds <= 0) return { points: 0, units: 0, reason: "" };
      return {
        points: personalRounds * tierPts,
        units: personalRounds,
        reason: "gauntlet rounds won",
      };
    }
    // No progression stored: only the final winner gets a fallback special.
    if (!isWinner) return { points: 0, units: 0, reason: "" };
    const fromResult = parseWinnerRoundWinsFromResult(match);
    const inferredRounds = matchData?.losers?.length ?? 0;
    const roundWins = Math.max(1, fromResult ?? inferredRounds);
    return { points: roundWins * tierPts, units: roundWins, reason: "gauntlet rounds won" };
  }

  if (!isWinner) return { points: 0, units: 0, reason: "" };

  if (isTwoOutOfThreeFallsLike(match)) {
    const roundWins = Math.max(1, parseWinnerRoundWinsFromResult(match) ?? 2);
    return { points: roundWins * tierPts, units: roundWins, reason: "2 out of 3 falls rounds won" };
  }

  return additionalOpponentVictoryBonus(match, matchData, tierPts);
}
...
/** Other tournament/PLE qualifiers that must not be scored as King/Queen of the Ring. */
function isNonKotrTournamentText(combined) {
  return (
    combined.includes("elimination chamber") ||
    combined.includes("money in the bank") ||
    /\bmitb\b/.test(combined)
  );
}
...
function isKingOfTheRingMatch(match) {
  ...
  // Chamber / MITB qualifiers are regular R/S matches, not KOTR/QOTR.
  if (isNonKotrTournamentText(combined)) return false;
  if (
    ...
    return true;
  // Loose fallback for short stip labels. Do NOT use bare "women" — that flagged Women's MITB qualifiers.
  if (
    (combined.includes("qualifier") || combined.includes("semi") || combined.includes("first round")) &&
    (/\bring\b/.test(combined) || combined.includes("king") || combined.includes("queen"))
  )
    return true;
  return false;
}
...
/**
 * Tag title DQ retention: Boxscore may set defendingChampion to one co-holder only.
 * If that slug is a winner and this wrestler is the other winning member of the same "(A & B)" team, they also retain (+2).
 */
function isCoWinningTagPartnerOfDefender(match, matchData, wrestlerSlugNorm, defendingSlugRaw) {
  const dSlug = slugifyName(String(defendingSlugRaw || "").trim());
  if (!dSlug || !wrestlerSlugNorm || dSlug === wrestlerSlugNorm) return false;
  const winners = matchData.winners || [];
  const defInWinners = winners.some((w) => slugifyName(String(w)) === dSlug);
  const selfInWinners = winners.some((w) => slugifyName(String(w)) === wrestlerSlugNorm);
  if (!defInWinners || !selfInWinners) return false;
  const raw = match.participants;
  const str = Array.isArray(raw) ? raw.join(" vs ") : String(raw || "");
  const teamParen = /\(([^)]+)\)/g;
  let m;
  while ((m = teamParen.exec(str)) !== null) {
    const inside = m[1];
    const names = inside.split(/[&,]/).map((s) => s.trim()).filter(Boolean);
    const slugs = names.map((n) => slugifyName(n)).filter(Boolean);
    if (slugs.includes(dSlug) && slugs.includes(wrestlerSlugNorm)) return true;
  }
  // Slug-only lineups (no "Team (A & B)" in participants): tag title, two unique winners = both retain on DQ
  const titleStr = getMatchTitle(match).toLowerCase();
  if (!titleStr.includes("tag")) return false;
  const winnerSlugs = [...new Set(winners.map((w) => slugifyName(String(w))).filter(Boolean))];
  if (winnerSlugs.length !== 2) return false;
  return winnerSlugs.includes(dSlug) && winnerSlugs.includes(wrestlerSlugNorm);
}
...
/** War Games entry bonus by position within each team (1st → 5th). */
const WAR_GAMES_ENTRY_POINTS_BY_TEAM_POSITION = [10, 8, 6, 4, 2];

function warGamesEntryPointsForTeamPosition(positionOnTeam) {
  if (positionOnTeam < 0 || positionOnTeam > 4) return 0;
  return WAR_GAMES_ENTRY_POINTS_BY_TEAM_POSITION[positionOnTeam];
}

/**
 * War Games entry order is per team: 1st on team = 10 pts, 2nd = 8, 3rd = 6, 4th = 4, 5th = 2.
 * Bron (1st for his team) = 10; CM Punk (1st for his team) = 10.
...
function getWarGamesEntryPoints(match, wrestlerName) {
  ...
  if (Array.isArray(entryOrderFromData) && entryOrderFromData.length > 0) {
    ...
    if (sameTeam.length > 0) {
      ...
      if (positionOnTeam >= 0 && positionOnTeam <= 4) {
        return warGamesEntryPointsForTeamPosition(positionOnTeam);
      }
    }
    return warGamesEntryPointsForTeamPosition((globalNum - 1) % 5);
  }
  ...
  if (fromResult.length >= 10) {
    ...
    if (index < 0) return 0;
    return warGamesEntryPointsForTeamPosition(index % 5);
  }
  if (fromResult.length > 0) {
    const index = fromResult.findIndex((entryName) => nameMatches(entryName, wrestlerName));
    if (index >= 0) return warGamesEntryPointsForTeamPosition(index % 5);
    return 0;
  }
...
/**
 * Count eliminations credited to a wrestler in a non–Royal Rumble battle royal.
 * Uses battleRoyalData.eliminations (Boxscore); falls back to royalRumbleData.eliminations shape if BR data absent.
 * Joint eliminations (eliminatedBy2) credit each eliminator with one elimination.
 */
function getBattleRoyalEliminatorCount(match, wrestlerName) {
  const br = match?.battleRoyalData ?? match?.battle_royal_data;
  const rrFallback = match?.royalRumbleData ?? match?.royal_rumble_data;
  const source =
    br && typeof br === "object" && Array.isArray(br.eliminations) && br.eliminations.length > 0
      ? br
      : rrFallback && typeof rrFallback === "object" && Array.isArray(rrFallback.eliminations)
        ? rrFallback
        : null;
  const raw = source?.eliminations ?? [];
  ...
  for (const row of raw) {
    ...
    const e1 = (row.eliminatedBy ?? row.eliminator ?? "").toString().trim();
    const e2 = (row.eliminatedBy2 ?? "").toString().trim();
    if (e1 && rrNameMatches(e1, wrestlerName)) count += 1;
    if (e2 && rrNameMatches(e2, wrestlerName)) count += 1;
  }
  return count;
}
...
export function calculateMatchPoints(match, event, allMatches, wrestlerName) {
  const points = {
    ...
    total: 0,
    breakdown: [],
    /** PLE tournament bonus on Raw/SmackDown (qualifier +3, semi +7). Counts as plePoints when the show is scored. */
    kotrPleBonus: 0,
    /** "king" | "queen" when kotrPleBonus > 0, for NOC participant lists. */
    kotrBracket: null,
    /** "first" | "semi" when kotrPleBonus > 0, for breakdown. */
    kotrRound: null,
  ...
  const isRRMatch = isRoyalRumbleMatch(match);
  const isTitle = isTitleMatch(match);
  const titleChanged = isTitleChange(match);

  const matchData = extractMatchParticipants(match);
  const wrestlerSlugNorm = slugifyName(wrestlerName);
  // Use exact slug or exact name match only for winner so similar names (e.g. El Grande Americano vs Original El Grande Americano) don't both get win points.
  const isWinner = matchData.winners.some(
    (w) => {
      ...
      if (wrestlerName.trim().toLowerCase() === wStr.toLowerCase()) return true;
      return false;
    }
  );
  const wLower = String(wrestlerName ?? "").toLowerCase();
  const isParticipant = matchData.participants.some((p) => {
    const ps = typeof p === "string" ? p : p != null ? String(p) : "";
    if (!ps.trim()) return false;
    const pLower = ps.toLowerCase();
    return pLower.includes(wLower) || wLower.includes(pLower);
  });
  ...
  // Generic battle royals only — not the Royal Rumble match (see EVENT_TYPES.ROYAL_RUMBLE block).
  if (isBR && !isRRMatch) {
    points.battleRoyalPoints += 1;
    points.breakdown.push("Battle Royal entry: +1");
    const elimCount = getBattleRoyalEliminatorCount(match, wrestlerName);
    if (elimCount > 0) {
      const elimPts = 2 * elimCount;
      points.battleRoyalPoints += elimPts;
      points.breakdown.push(`Battle Royal eliminations (${elimCount}): +${elimPts}`);
    }
    if (isWinner) {
  ...
  if (isTitle) {
    const eventDateYmd = String(event?.date ?? "").trim().slice(0, 10);
    const matchTitleLabel = getMatchTitle(match) || String(match.title ?? match.Title ?? "").trim();
    const titleScoringRetired =
      Boolean(matchTitleLabel) && isChampionshipRetiredForScoringDate(matchTitleLabel, eventDateYmd);

    if (!titleScoringRetired) {
    const isNxtScoringEvent =
      eventType === EVENT_TYPES.NXT || String(eventType || "").startsWith("nxt-");
    const titleWinPoints = isNxtScoringEvent ? 2 : 5;
    const titleDefensePoints = isNxtScoringEvent ? 1 : 4;
    const titleDefenseDqPoints = isNxtScoringEvent ? 0.5 : 2;
    if (titleChanged && isWinner) {
      points.titlePoints += titleWinPoints;
      points.breakdown.push(`Title win: +${titleWinPoints}`);
      ...
      if (isDQResult) {
        const defendingSlug =
          (match.defendingChampion ?? match.defending_champion ?? "")
            .toString()
            .trim();
        const dSlug = defendingSlug ? slugifyName(defendingSlug) : "";
        ...
        const participantSlugs = new Set(
          (matchData.participants || [])
            .map((p) => {
              const ps = typeof p === "string" ? p : p != null ? String(p) : "";
              return slugifyName(ps);
            })
            .filter(Boolean)
        );
        // Boxscore sometimes sends a team name ("The Irresistible Forces") instead of a wrestler slug;
        // that slug never matches participant slugs — treat like unspecified: all winners retain.
        const defenderSlugMatchesSomeWinner =
          Boolean(dSlug) && winnerSlugs.has(dSlug);
        const defenderSlugMatchesParticipant =
          Boolean(dSlug) && participantSlugs.has(dSlug);
        let isDefendingChampion;
        if (!dSlug) {
          isDefendingChampion = isWinner;
        } else if (wrestlerSlugNorm && dSlug === wrestlerSlugNorm) {
          isDefendingChampion = true;
        } else if (!defenderSlugMatchesParticipant && isWinner) {
          isDefendingChampion = true;
        } else if (isWinner && defenderSlugMatchesSomeWinner) {
          isDefendingChampion = isCoWinningTagPartnerOfDefender(
            match,
            matchData,
            wrestlerSlugNorm,
            defendingSlug
        ...
        if (isDefendingChampion) {
          points.titlePoints += titleDefenseDqPoints;
          points.breakdown.push(`Title defense (DQ): +${titleDefenseDqPoints}`);
        }
      } else if (isWinner) {
        points.titlePoints += titleDefensePoints;
        points.breakdown.push(`Title defense: +${titleDefensePoints}`);
      }
    }
    }
  }
  ...
  const specialVictoryBonus = calculateSpecialVictoryBonus(
    match,
    matchData,
    eventType,
    isWinner,
  ...
  if (specialVictoryBonus.points > 0) {
    points.specialPoints += specialVictoryBonus.points;
    const each = getVictoryBonusTierPoints(eventType);
    points.breakdown.push(
      `Special match victory bonus (${specialVictoryBonus.reason}: ${specialVictoryBonus.units} × ${each}): +${specialVictoryBonus.points}`
    );
  }
  ...
  if (isRS) {
    ...
    if (knownSlot) {
      points.kotrPleBonus = 7;
      points.kotrBracket = knownSlot.bracket;
      points.kotrRound = "semi";
      points.breakdown.push("King/Queen of the Ring semi-final: +7 (PLE)");
      ...
      if (kotrRound === "first") {
        points.kotrPleBonus = 3;
        points.kotrBracket = bracket;
        points.kotrRound = "first";
        points.breakdown.push("King/Queen of the Ring qualifier: +3 (PLE)");
  ...
  // KOTR qualifier/semi PLE bonus stacks with Raw/SmackDown appearance and win points.
  points.total =
    points.matchPoints +
    points.titlePoints +
    ...
    (points.kotrPleBonus || 0);

  // Legacy alias for NOC tournament tables and older callers.
  points.kotrTowardNOC = points.kotrPleBonus || 0;
...
function calculateEventSpecificPoints(
  ...
  // Generic battle royal (not the Royal Rumble match): battleRoyalPoints already awards entry, eliminations,
  // and winner. Do not also apply Raw/SmackDown or PLE "on card" / "winning match" / main-event points.
  if (isBattleRoyal(match) && !isRoyalRumbleMatch(match)) {
    return points;
  }

  if (eventType === EVENT_TYPES.NXT) {
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 1 : 2;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning NXT main event (DQ): +1" : "Winning NXT main event: +2");
      }
      points.mainEventPoints += 1.5;
      points.breakdown.push("Main eventing NXT: +1.5");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 0.5 : 1;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning NXT match (DQ): +0.5" : "Winning NXT match: +1");
      }
      points.matchPoints += 0.5;
      points.breakdown.push("On NXT card: +0.5");
    }
    return points;
  }

  if (eventType === EVENT_TYPES.NXT_STAND_AND_DELIVER) {
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 5 : 10;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning Stand & Deliver main event (DQ): +5" : "Winning Stand & Deliver main event: +10");
      }
      points.mainEventPoints += 8;
      points.breakdown.push("Main eventing Stand & Deliver: +8");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 2.5 : 5;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning Stand & Deliver match (DQ): +2.5" : "Winning Stand & Deliver match: +5");
      }
      points.matchPoints += 4;
      points.breakdown.push("On Stand & Deliver card: +4");
    }
    return points;
  }
  ...
  if (
    ...
    const isSpecialEpisode =
      eventType === EVENT_TYPES.NXT_GOLD_RUSH ||
      eventType === EVENT_TYPES.NXT_HOMECOMING ||
      eventType === EVENT_TYPES.NXT_NEW_YEARS_EVIL ||
      eventType === EVENT_TYPES.NXT_REVENGE ||
      eventType === EVENT_TYPES.NXT_ROADBLOCK ||
      eventType === EVENT_TYPES.NXT_SHOWDOWN;
    const mainEventAppearance = isSpecialEpisode ? 2 : 3;
    const mainEventWin = isSpecialEpisode ? 3 : 4;
    const onCardPts = isSpecialEpisode ? 1 : 2;
    const winPts = isSpecialEpisode ? 2 : 3;
    if (isMain) {
      if (isWinner) {
        const awarded = isDQ ? mainEventWin / 2 : mainEventWin;
        points.matchPoints += awarded;
        points.breakdown.push(
          isDQ
            ? `Winning NXT ${isSpecialEpisode ? "special episode" : "special"} main event (DQ): +${awarded}`
            : `Winning NXT ${isSpecialEpisode ? "special episode" : "special"} main event: +${awarded}`
        );
      }
      points.mainEventPoints += mainEventAppearance;
      points.breakdown.push(
        `Main eventing NXT ${isSpecialEpisode ? "special episode" : "special"}: +${mainEventAppearance}`
      );
    } else {
      if (isWinner && !isNC) {
        const awarded = isDQ ? winPts / 2 : winPts;
        points.matchPoints += awarded;
        points.breakdown.push(
          isDQ
            ? `Winning NXT ${isSpecialEpisode ? "special episode" : "special"} match (DQ): +${awarded}`
            : `Winning NXT ${isSpecialEpisode ? "special episode" : "special"} match: +${awarded}`
        );
      }
      points.matchPoints += onCardPts;
      points.breakdown.push(
        `On NXT ${isSpecialEpisode ? "special episode" : "special"} card: +${onCardPts}`
      );
    }
    return points;
  }
  ...
  if (eventType === EVENT_TYPES.WRESTLEMANIA_NIGHT_1) {
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 15 : 30;
        points.matchPoints += winPts;
        points.breakdown.push(
          isDQ
            ? "Winning Night One Main Event at WrestleMania (DQ): +15"
            : "Winning Night One Main Event at WrestleMania: +30"
        );
      }
      points.mainEventPoints += 25;
      points.breakdown.push("Main Eventing Night One at WrestleMania: +25");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 8 : 16;
        points.matchPoints += winPts;
        points.breakdown.push(
          isDQ ? "Winning Non-ME Match at WrestleMania (DQ): +8" : "Winning Non-ME Match at WrestleMania: +16"
        );
      }
      points.matchPoints += 8;
      points.breakdown.push("Being on the Non-ME Card at WrestleMania: +8");
    }
    return points;
  }
  ...
  if (eventType === EVENT_TYPES.WRESTLEMANIA_NIGHT_2) {
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 20 : 40;
        points.matchPoints += winPts;
        points.breakdown.push(
          isDQ
            ? "Winning Main Event Night Two at WrestleMania (DQ): +20"
            : "Winning Main Event Night Two at WrestleMania: +40"
        );
      }
      points.mainEventPoints += 30;
      points.breakdown.push("Main Eventing Night Two at WrestleMania: +30");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 8 : 16;
        points.matchPoints += winPts;
  ...
  if (
    ...
    if (isMain) {
      if (isWinner) {
        if (eventType === EVENT_TYPES.SUMMERSLAM_NIGHT_1) {
          const winPts = isDQ ? 12 : 25;
          points.matchPoints += winPts;
          points.breakdown.push(
            isDQ
              ? "Winning SummerSlam Night 1 main event (DQ): +12"
              : "Winning SummerSlam Night 1 main event: +25"
          );
        } else {
          const winPts = isDQ ? 15 : 30;
          points.matchPoints += winPts;
          points.breakdown.push(
            isDQ
              ? "Winning SummerSlam Night 2 main event (DQ): +15"
              : "Winning SummerSlam Night 2 main event: +30"
          );
        }
      }
      if (eventType === EVENT_TYPES.SUMMERSLAM_NIGHT_2) {
        points.mainEventPoints += 30;
        points.breakdown.push("Main eventing SummerSlam Night 2: +30");
      } else {
        points.mainEventPoints += 20;
        points.breakdown.push("Main eventing SummerSlam Night 1: +20");
      }
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 10 : 20;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning SummerSlam match (DQ): +10" : "Winning SummerSlam match: +20");
      }
      points.matchPoints += 10;
      points.breakdown.push("On SummerSlam card: +10");
    }
    return points;
  }
  ...
  if (eventType === EVENT_TYPES.SURVIVOR_SERIES) {
    ...
    if (isWarGames) {
      // War Games: own scoring. No generic "on card" or "main event" — team 15, win 30, pinfall 20, entry 10/8/6/4/2.
      points.specialPoints += 15;
      points.breakdown.push("War Games team: +15");
      if (isWinner && !isNC) {
        points.specialPoints += 30;
        points.breakdown.push("Winning War Games: +30");
      }
      ...
      if (pinfallWinner && nameMatches(pinfallWinner, wrestlerName)) {
        ...
        points.breakdown.push("War Games pinfall: +20");
      }
      const entryPoints = getWarGamesEntryPoints(match, wrestlerName);
      if (entryPoints > 0) {
        points.specialPoints += entryPoints;
        const entryNumber = (10 - entryPoints) / 2 + 1;
        points.breakdown.push(`War Games entry #${entryNumber}: +${entryPoints}`);
      }
    } else if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 15 : 30;
        points.matchPoints += winPts;
        points.breakdown.push(
          isDQ ? "Winning Survivor Series main event (DQ): +15" : "Winning Survivor Series main event: +30"
        );
      }
      points.mainEventPoints += 30;
      points.breakdown.push("Main eventing Survivor Series: +30");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 10 : 20;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning Survivor Series match (DQ): +10" : "Winning Survivor Series match: +20");
      }
      points.matchPoints += 10;
      points.breakdown.push("On Survivor Series card: +10");
    }
    return points;
  }
  ...
  if (
    ...
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 8 : 16;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning Night of Champions main event (DQ): +8" : "Winning Night of Champions main event: +16");
      }
      points.mainEventPoints += 12;
      points.breakdown.push("Main eventing Night of Champions: +12");
    } else {
      if (isWinner && !isNC) {
        const winPts = isDQ ? 5 : 10;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning Night of Champions match (DQ): +5" : "Winning Night of Champions match: +10");
      }
      points.matchPoints += 5;
      points.breakdown.push("On Night of Champions card: +5");
    }
    return points;
  }
  ...
  if (eventType === EVENT_TYPES.MONEY_IN_THE_BANK) {
    ...
    if (isMITBMatch) {
      const isLadder =
        matchType.includes("ladder") ||
        match.stipulation?.toLowerCase().includes("ladder");
      const isQualifier = match.stipulation?.toLowerCase().includes("qualifier");
      if (isLadder || isQualifier) {
        points.specialPoints += 15;
        points.breakdown.push("MITB ladder match participant: +15");
      }
      if (isWinner) {
        points.specialPoints += 30;
        points.breakdown.push("Money in the Bank winner: +30");
      }
    }
    ...
    if (isMain) {
      if (isWinner && !isMITBMatch) {
        const winPts = isDQ ? 8 : 16;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning MITB main event (DQ): +8" : "Winning MITB main event: +16");
      }
      if (!isMITBMatch) {
        points.mainEventPoints += 12;
        points.breakdown.push("Main eventing MITB: +12");
      }
    } else {
      if (isWinner && !isNC && !isMITBMatch) {
        const winPts = isDQ ? 5 : 10;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning MITB match (DQ): +5" : "Winning MITB match: +10");
      }
      if (!isMITBMatch) {
        points.matchPoints += 5;
        points.breakdown.push("On MITB card: +5");
      }
    }
    return points;
  }
  ...
  if (
    ...
    if (isMain) {
      ...
      points.mainEventPoints += 9;
      points.breakdown.push("Main eventing minor PLE: +9");
    } else {
      if (isWinner && !isNC) {
