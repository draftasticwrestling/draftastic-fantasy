...
export function calculateMatchPoints(match, event, allMatches, wrestlerName) {
  ...
  const defendingRaw = (match.defendingChampion ?? match.defending_champion ?? "")
    .toString()
    .trim();
  const defendingSlug = defendingRaw ? slugifyName(defendingRaw) : "";
  let effectiveTitleChanged = isTitleChange(match);
  if (isTitle && isWinner && defendingSlug && wrestlerSlugNorm) {
    // When boxscore lists the defending champion, prefer that over titleOutcome text (often wrong on new reigns).
    effectiveTitleChanged = defendingSlug !== wrestlerSlugNorm;
  }
  ...
  if (isTitle) {
    if (effectiveTitleChanged && isWinner) {
      points.titlePoints += 5;
      points.breakdown.push("Title win: +5");
    } else if (!effectiveTitleChanged) {
...
function calculateEventSpecificPoints(
  ...
  if (eventType === EVENT_TYPES.RAW || eventType === EVENT_TYPES.SMACKDOWN) {
    ...
    if (isTitle && effectiveTitleChanged && isWinner) {
