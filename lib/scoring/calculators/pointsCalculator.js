  ...
  if (
    ...
    if (isMain) {
      if (isWinner) {
        const winPts = isDQ ? 15 : 30;
        points.matchPoints += winPts;
        points.breakdown.push(isDQ ? "Winning SummerSlam main event (DQ): +15" : "Winning SummerSlam main event: +30");
      }
      if (eventType === EVENT_TYPES.SUMMERSLAM_NIGHT_2) {
        points.mainEventPoints += 25;
        points.breakdown.push("Main eventing SummerSlam Night 2: +25");
      } else {
