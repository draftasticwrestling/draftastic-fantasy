 ...
 *   - From 2026-01-31 (Royal Rumble): Chad Gable (gimmick retired — Chad Gable is the roster/list row)
 * - Bravo Americano:
...
function normWrestlerSlug(wrestlerSlug) {
  if (!wrestlerSlug || typeof wrestlerSlug !== "string") return "";
  return normalizeWrestlerName(wrestlerSlug) || wrestlerSlug.toLowerCase().trim();
}
 ...
 * Active gimmick rows (Rayo, Bravo, EGA, Bruto/Julio Credo) stay visible; points merge via draftAliasListMerge.
 * Retired gimmicks (e.g. Original El Grande Americano) are hidden — show the performer instead.
 */
...
/** Retired/inactive persona rows — roster and lists use the performer (Chad Gable, etc.). */
const HIDDEN_RETIRED_PERSONA_SLUGS = new Set(["original-el-grande-americano"]);
...
export function isHiddenCanonicalListSlug(wrestlerSlug) {
  const slug = normWrestlerSlug(wrestlerSlug);
  if (!slug) return false;
  return HIDDEN_PRIMARY_LIST_CANONICAL_SLUGS.has(slug) || HIDDEN_RETIRED_PERSONA_SLUGS.has(slug);
}
...
export function getPersonasForDisplay(canonicalSlug) {
  ...
  if (slug === "chad-gable") {
    return "Also: El Grande Americano (through Jun 2025); formerly Original El Grande Americano (Jan 2026)";
  }
