...
export function isTitleChange(match) {
  ...
  if (o === "new champion") return true;
  // Broader phrasing from boxscore feeds (avoid false positives)
  if (
    o.includes("no new champion") ||
    o.includes("no title change") ||
    o.includes("does not change")
  ) {
    return false;
  }
  if (o.includes("new champion")) return true;
  if (o.includes("champion crowned") || o.includes("crowned new")) return true;
  if (o.includes("title changes hands") || o.includes("title change")) return true;
  return false;
}
