...
/**
 * Weekly PST title-hold points for one week-ending Sunday.
 ...
 * @param {string} weekEndSundayYmd
 * @param {string} firstEligibleWeekEndSunday
 */
export function computeWeeklyBeltHoldPointsForWeekEndSunday(reigns, weekEndSundayYmd, firstEligibleWeekEndSunday) {
  const bySlug = /** @type {Record<string, number>} */ ({});

  if (!reigns?.length || !weekEndSundayYmd || !firstEligibleWeekEndSunday) return bySlug;
  if (weekEndSundayYmd < firstEligibleWeekEndSunday || weekEndSundayYmd < REIGN_EFFECTIVE_START) return bySlug;
  ...
  for (const reign of reigns) {
    ...
    if (won > weekEndSundayYmd || (lost != null && lost <= weekEndSundayYmd)) continue;
