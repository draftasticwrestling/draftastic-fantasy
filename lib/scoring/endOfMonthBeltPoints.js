...
import {
  ...
  isTagTeamTitle,
  normalizeChampionshipTitleForScoring,
...
import { formatChampionshipTitleForHolder, normalizeReignKind } from "../championshipReignKind";
import { isChampionshipRetiredAsOf, isChampionshipRetiredForScoringDate } from "../retiredChampionships.js";
...
/**
 * Completed event rows for inferReignsFromEvents, aligned with getLeagueScoring.
 * Optional league end caps the timeline for closed leagues.
 * @param {Array<{ date?: string | null }> | null | undefined} events
 * @param {string | null | undefined} leagueEndYmd
 * @returns {Array<{ date?: string | null }>}
 */
export function filterEventsForBeltReignInference(events, leagueEndYmd) {
  const cap = leagueEndYmd ? String(leagueEndYmd).slice(0, 10) : "";
  return (events ?? []).filter((e) => {
    const d = String(e.date ?? "").slice(0, 10);
    if (!d || d < BELT_REIGN_INFERENCE_EVENTS_FROM) return false;
    if (cap && /^\d{4}-\d{2}-\d{2}$/.test(cap) && d > cap) return false;
    return true;
  });
}
...
/**
 * NXT Men's / Women's Speed: 1 pt per calendar month hold, 0.25 pt per Mon–Sun week (not the default tier).
 * Matches PWBS-style names and short Boxscore labels (e.g. "Men's Speed Championship" without "NXT").
 * @param {string} t - `normalizeChampionshipTitleForScoring` output
 */
function isNxtSpeedChampionshipTitleNormalized(t) {
  if (!t) return false;
  return (
    /\bnxt\s+(men'?s?|women'?s?)\s+speed\b/.test(t) ||
    /\b(men'?s?|women'?s?)\s+speed\s+championship\b/.test(t)
  );
}
 ...
 * NXT Speed is handled above (1 / month, 0.25 / week), not in this table.
...
/**
 * Tier points for a title (12 / 8 / 4 / default), except NXT Speed = 1/month.
 * Weekly hold scoring uses getWeeklyHoldPointsForTitle (generally one quarter per week: 3 / 2 / 1; Speed = 0.25).
...
export function getBeltPointsForTitle(titleName) {
  ...
  if (isNxtSpeedChampionshipTitleNormalized(t)) return 1;
...
/**
 * Fantasy points credited each week (Mon–Sun) for holding the title.
 * 12-point tier → 3/week, 8 → 2, 4 → 1; NXT Speed → 0.25; default tier uses month/4 (e.g. 1.25 from 5).
...
export function getWeeklyHoldPointsForTitle(titleName) {
  if (titleName && typeof titleName === "string") {
    const t = normalizeChampionshipTitleForScoring(titleName);
    if (isNxtSpeedChampionshipTitleNormalized(t)) return 0.25;
  }
...
export function computeWeeklyBeltHoldPointsAccumulated(reigns, firstWeekEndSunday, lastWeekEndSundayCap, nowMs = Date.now()) {
  ...
  for (const reign of reigns) {
    ...
    for (const weekEnd of weekEndDates) {
      ...
      if (isChampionshipRetiredForScoringDate(titleName, weekEnd)) continue;
...
/**
 * Weekly PST title-hold points for one snapshot date (who held at end of that calendar day once that week’s
 * final show is marked completed in PWBS).
 ...
 * @param {string} lockYmd - Calendar date of the last show in the Mon–Sun week (or week-ending Sunday when no rows exist).
...
export function computeWeeklyBeltHoldPointsForWeekEndSunday(
  ...
  for (const reign of reigns) {
    ...
    const key = canonicalSlug(slug);
    const titleName = reignTitleLabel(reign);
    if (isChampionshipRetiredForScoringDate(titleName, lockYmd)) continue;
    const points = getWeeklyHoldPointsForTitle(titleName);
    addMonthlyBeltPointsForReignRow(bySlug, {
      key,
      slug,
...
export function computeEndOfMonthBeltPoints(reigns, firstMonthEnd, lastMonthEnd, nowMs = Date.now()) {
  ...
  for (const reign of reigns) {
    ...
    for (const monthEnd of monthEndDates) {
      ...
      if (isChampionshipRetiredForScoringDate(titleName, monthEnd)) continue;
...
export function computeEndOfMonthBeltPointsForSingleMonth(reigns, monthEndDate, firstMonthEnd) {
  ...
  for (const reign of reigns) {
    ...
    const key = canonicalSlug(slug);
    const titleName = reignTitleLabel(reign);
    if (isChampionshipRetiredForScoringDate(titleName, monthEndDate)) continue;
    const points = getBeltPointsForTitle(titleName);
    addMonthlyBeltPointsForReignRow(bySlug, {
      key,
      slug,
...
export function getCurrentChampionsBySlug(reigns) {
  ...
  for (const reign of reigns) {
    ...
    const slug = resolvePersonaToCanonical(rawSlug, won) ?? rawSlug;
    const key = canonicalSlug(slug);
    const baseTitle = reignTitleLabel(reign).trim() || "Championship";
    const champId = (reign.championship_id ?? "").toString().trim();
    // Retired belts keep history but are not "current" holders after the retirement date.
    if (
      isChampionshipRetiredAsOf(baseTitle, today) ||
      (champId && isChampionshipRetiredAsOf(champId, today))
    ) {
      continue;
    }
    const titleName = formatChampionshipTitleForHolder(
      baseTitle,
      normalizeReignKind(reign.reign_kind ?? reign.reignKind)
    );
