...
function titleGroupKey(title) {
  ...
  if (!t) return "";
  if (
    /undisputed\s+wwe|wwe\s+undisputed|wwe\s+championship(?!\s+women)/.test(t) &&
    !/tag/i.test(t)
  ) {
  ...
  // Tag titles: each belt must have its own succession chain. A single "tag-team" bucket wrongly
  // closed e.g. World Tag reigns when Raw Tag changed hands, and mixed women's with men's tags.
  if (/women'?s?\s+tag\b|wwe\s+women'?s?\s+tag\b/i.test(t)) {
    return "womens-tag-team";
  }
  if (/\braw\s+tag\b/i.test(t)) {
    return "raw-tag-team";
  }
  if (/\bsmackdown\s+tag\b/i.test(t)) {
    return "smackdown-tag-team";
  }
  if (
    /world\s+tag\b/i.test(t) ||
    (/undisputed/.test(t) && /tag/i.test(t))
  ) {
    return "world-undisputed-tag-team";
  }
  // Standalone "WWE Tag Team Championship" (no Raw/SD/world prefix) — same lineage as world/SD in modern data.
  if (/\bwwe\s+tag\s+team\b/i.test(t) && !/raw|smackdown|women/i.test(t)) {
    return "world-undisputed-tag-team";
  }
  if (/tag\s+team/i.test(t)) {
    return "generic-tag-team";
  }
