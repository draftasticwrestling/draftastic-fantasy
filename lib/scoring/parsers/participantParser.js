...
/**
 * Split a string into team segments for multi-team result lines.
 * Splits by: (1) ") & " — after a named team's closing paren; (2) " & (" — before a named team's opening paren;
 * (3) " & " when followed by "TeamName (" — so "A & B & Team (x & y)" splits into "A & B" and "Team (x & y)".
 * Prevents "Lash Legend & Nia Jax & Allies of Convenience (Charlotte & Alexa)" being one segment.
 */
function splitMultiTeamByAmpersand(str) {
  if (!str || typeof str !== "string") return [];
  const trimmed = str.trim();
  if (!trimmed) return [];
  const segments = [];
  let start = 0;
  // Use RegExp constructor to avoid bundler mangling. (3) = " & " with lookahead for team name then optional space + "(".
  const re = new RegExp("\\)\\s+&\\s+|\\s+&\\s+(?=[^&]*\\s*\\()|\\s+&\\s+\\(", "g");
  let match;
  while ((match = re.exec(trimmed)) !== null) {
    if (match[0].startsWith(")")) {
      segments.push(trimmed.slice(start, match.index + 1).trim());
    } else {
      segments.push(trimmed.slice(start, match.index).trim());
    }
...
export function parseParticipants(participantsStr) {
  ...
  for (const part of parts) {
    let segments;
    // "Team1 (x) and Team2 (y)" — split on ") and " when followed by another team name and " (" so each segment is "Team (members)"
    if (part.includes(") and ") && part.includes(" (")) {
      const andBetweenTeamsRe = new RegExp("\\)\\s+and\\s+(?=[^)]+\\s+\\()", "g");
      const andParts = part.split(andBetweenTeamsRe).map((s) => s.trim()).filter(Boolean);
      for (let i = 0; i < andParts.length - 1; i++) andParts[i] += ")";
      segments = andParts;
    } else {
      // Multi-team string: split by ") & " or " & (" so each team is one segment.
      const needsMultiTeamSplit =
        part.includes(") & ") ||
        part.includes(") &") ||
        part.includes(" & (") ||
        (part.includes(" & ") && part.includes("("));
      segments = needsMultiTeamSplit ? splitMultiTeamByAmpersand(part) : [part];
    }

    for (const segment of segments) {
      const teamMatch = segment.match(/^(.+?)\s*\(([^)]+)\)$/);
      if (teamMatch) {
        ...
        if (trimmed.includes(" & ")) {
          ...
          // Gauntlet / entry order: "Lyra Valkyria → Iyo Sky → Raquel Rodriguez → ..."
          const members = trimmed
            .split(/\s*[→]\s*|\s+->\s+/)
            .map((m) => m.trim())
            .filter(Boolean);
...
export function parseWinnersAndLosers(resultStr, participantsStr) {
  ...
  if (defMatch) {
    ...
    const loserStart = lowerResult.indexOf(defMatch[2]);
    const loserPart = (resultStr || "").slice(loserStart, loserStart + defMatch[2].length).trim();
    // Strip trailing method (e.g. " · Pinfall", " - Pinfall", " – Submission") so team regex can match "Team (x & y)"
    const loserPartWithoutMethod = loserPart
      .replace(/\s+[·\-–]\s+\S.*$/, "")
      .trim();
    // Gauntlet / multi-way: "Winner def. A, B, C, D" → parse each loser so every participant has an outcome.
    // Each name (e.g. "Bron Breakker", "Penta", "LA Knight", "Jey Uso") becomes one entry in losers; winner is already in winners.
    // This ensures all five (or N) participants are in winnerSlugs or loserSlugs so no one is marked "Matches needing review".
    const losers = loserPartWithoutMethod.includes(",")
      ? loserPartWithoutMethod
          .split(",")
          .map((s) => s.trim())
          .filter(Boolean)
          .flatMap((name) => parseParticipants(name).map((p) => p.name || p))
      : parseParticipants(loserPartWithoutMethod).map((p) => p.name || p);
  ...
  // Fallback is normal for many Boxscore rows (commas, vacated lines, promos, etc.).
  // Avoid logger.warn here: bulk callers (wrestlers page, aggregates) hit this thousands of times and stall the dev server.
  if (participantsStr && !resultStr) {
    logger.debug(`Could not parse result, using participants: ${participantsStr}`);
    result.unclear = parseParticipants(participantsStr).map((p) => p.name || p);
  } else {
    logger.debug(`Could not parse result string: ${resultStr}`);
...
export function extractMatchParticipants(match) {
  ...
  // or side strings ("A & B & C", "Team (x & y)") — expand through parseParticipants.
  if (Array.isArray(rawParticipants) && rawParticipants.length > 0) {
    const slugList = rawParticipants.flatMap((p) => {
      const s = (typeof p === "string" ? p : String(p)).trim();
      if (!s) return [];
      const parsed = parseParticipants(s);
      const individuals = parsed
        .filter((x) => x.type === "individual")
        .map((x) => x.name || x)
        .filter(Boolean);
      return individuals.length > 0 ? individuals : [s];
    });
    participantsForScoring = dedupeExpandedParticipantNames(slugList);
    participants = participantsForScoring.map((name) => ({ type: "individual", name }));
  } else {
    const participantsStr = rawParticipants || "";
    participants = parseParticipants(participantsStr);
    participantsForScoring = dedupeExpandedParticipantNames(
      participants.filter((p) => p.type === "individual").map((p) => p.name || p)
    );
  }
  ...
  const teamNameToMembers = new Map();
  for (const p of participants || []) {
    if (p.type === "team" && p.name && Array.isArray(p.members) && p.members.length > 0) {
      const key = normalizeWrestlerName(p.name);
      if (key) teamNameToMembers.set(key, p.members);
    }
  }
  function expandTeamNames(names) {
    const out = [];
    for (const n of names) {
      const key = normalizeWrestlerName(String(n));
      const members = key ? teamNameToMembers.get(key) : null;
      if (members && members.length > 0) {
        out.push(...members);
      } else {
        out.push(typeof n === "string" ? n : (n && n.name) || n);
      }
    }
    return out;
  }
  if (winners.length > 0 || losers.length > 0) {
    winners = expandTeamNames(winners);
    losers = expandTeamNames(losers);
  }
  ...
  return {
    ...
    winners: dedupeExpandedParticipantNames(winners),
    losers: dedupeExpandedParticipantNames(losers),
    unclear,
    hasResult: !!resultStr,
...
/**
 * After expandTeamNames, the same person can appear twice: parseParticipants adds both the team
 * label and each member to winners/losers, then expansion injects members again.
 */
function dedupeExpandedParticipantNames(names) {
  const seen = new Set();
  const out = [];
  for (const n of names || []) {
    const label = typeof n === "string" ? n : (n && n.name) || n;
    const key = normalizeWrestlerName(String(label));
