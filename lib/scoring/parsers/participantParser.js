 ...
 * Splits by: (1) ") & " — after a named team's closing paren, or (2) " & (" — before a named team's opening paren.
 * So "Team A (a & b) & Team B (c & d)" and "Bravo & Rayo & Team C (x & y)" both parse into separate teams;
 * "Bravo Americano & Rayo Americano" (no parens) stays one segment (two-person team).
 */
function splitMultiTeamByAmpersand(str) {
  ...
  const segments = [];
  let start = 0;
  // Use RegExp constructor to avoid bundler mangling regex literal with \)
