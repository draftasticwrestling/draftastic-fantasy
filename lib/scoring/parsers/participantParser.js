...
function splitMultiTeamByAmpersand(str) {
  ...
  const segments = [];
  let start = 0;
  // Use RegExp constructor to avoid bundler mangling. (3) = " & " with lookahead for team name then " (".
  ...
  for (const part of parts) {
    ...
    const segments =
      part.includes(") & ") || part.includes(") &") || part.includes(" & (")
        ? splitMultiTeamByAmpersand(part)
        : [part];
...
export function parseWinnersAndLosers(resultStr, participantsStr) {
  ...
  if (defMatch) {
    ...
    const loserPartWithoutMethod = loserPart.split(/\s+-\s+/)[0].trim();
