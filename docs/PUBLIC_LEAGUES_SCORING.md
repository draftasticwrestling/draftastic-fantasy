...
Implementation: `lib/leagues.ts` uses `getRosterStintsForLeague()` (each stint = one `league_rosters` row with `acquired_at` / `acquired_at_ts` and `released_at` / `released_at_ts`). For each event, a wrestler's points are added to an owner only if the stint covers the event **calendar date** and, when the event has `broadcast_start_ts`, roster timestamps (when stored) are compared directly to that instant so you must **have the wrestler at show start** (same-day drop before airtime does not score for the old team; pickup before airtime scores for the new team; release **after** the show still scores for that event even if the release instant is the next UTC calendar day). Without `broadcast_start_ts`, legacy end-of-day UTC windows apply. Trades and drops set `released_at` on the old stint and create a new stint for the new team; FA signings create a new stint with `acquired_at` = date of signing.
