...
- **Free agent signings (Road to SummerSlam / Survivor Series):** Each faction may complete **up to 2 free agent signings** **per week** (Monday–Sunday in **Pacific Time**, America/Los_Angeles; half-open window Monday 00:00 PT through next Monday 00:00 PT). **Standalone drops do not count** toward this cap. **Trades are unlimited.** Signings are **not** allowed during an event (same lock rules as other roster changes).
- **Salary cap leagues:** Each faction may **sign up to $25** and **drop up to $25** in wrestler salary value **per week** (same Monday PT window). Values use `wrestlers.salary_cap_cost` on each `fa_add` / `drop` in `league_activity` (swap signings count both the add and the paired drop). Moves can be spread across the week. **Trades are unlimited.** Enforcement: `lib/salaryCapWeeklyLimits.ts`.
  - **Open:** From the **end of the last weekly event** (e.g. SmackDown or that week’s PLE) until the next lock.
  - **Lock:** Rosters lock **1 hour before** each event. While locked, no FA signings and no line-up changes for that event.
- Summary: Line-up lock = 30 min before event; full roster lock (FA + line-up) = 1 hour before event. FA window runs from end of last event until 1 hour before the next event. Enforcement counts `fa_add` in `league_activity` (`lib/freeAgentSigningLimits.ts`). Lock constants in `lib/leagueStructure.ts` and `lib/publicLeagueRosterRules.ts`.
...
Implementation: `lib/leagues.ts` uses `getRosterStintsForLeague()` (each stint = one `league_rosters` row with `acquired_at` / `acquired_at_ts` and `released_at` / `released_at_ts`). The `*_at_ts` values are written so their **UTC calendar date** matches the date column (see `lib/rosterTimestamps.ts`); backdated commissioner adds use noon UTC on that date instead of “now”. For each event with `broadcast_start_ts`, a stint scores only if the calendar range covers the event **and** the wrestler was acquired on or before broadcast start and not released on or before broadcast start (using timestamps when present). Without `broadcast_start_ts`, legacy end-of-event-day UTC comparisons apply.
